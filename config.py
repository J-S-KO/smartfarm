...
LED_OFF_HOUR = 20          # 20:00 OFF (10분 동안 페이드 아웃) - 하루 종일 켜두기 위해 8시에서 20시로 변경
...
# 5. 🌊 VPD 기반 자동 밸브 제어 (미스트 효과)
# 세트 개념: 3분 ON + 2분 OFF = 1 사이클, 3 사이클 = 1 세트 (총 15분)
# 세트 반복 조건: 이전 세트 종료 VPD보다 0.3 이상 올라가면 다음 세트 시작
VPD_VALVE_THRESHOLD = 2.0           # VPD 임계값 (이 값 초과 시 첫 세트 시작)
VALVE_ON_DURATION = 180             # 밸브 ON 시간 (초, 3분 = 180초)
VALVE_OFF_DURATION = 120            # 밸브 OFF 시간 (초, 2분 = 120초)
VPD_VALVE_CYCLES_PER_SET = 3        # 1 세트당 사이클 횟수 (3 사이클 = 1 세트)
VPD_VALVE_SET_VPD_INCREASE = 0.3    # 세트 반복 조건 (이전 세트 종료 VPD + 이 값 이상이면 다음 세트 시작)
VPD_VALVE_ONCE_PER_DAY = True       # 첫 세트만 1일 1회 제한 (True: 첫 세트만 1일 1회, False: 제한 없음)
